...
const FactureFournisseur: React.FC = () => {

  const AddFacture: React.FC = () => {
    const [data, setData] = useState<[{}]>([{}]);
    const [dataFourn, setDataFourn] = useState<[{}]>([{}]);
    ...
    const account_compte=["21","60","61","62"]
...
//--------------------------get journal--------------------------------------
    ...
    const JournalSelected = data.map((e: any) => <Option value={e.id} key={e.id}>{e.code}</Option>)
    const defaultOption = {
      key: data[0].id,
      value: data[0].code
    };
    console.log("defaultOption aaaaaaaaa",defaultOption);
    ...
    useEffect(() => {
      axios.get(`${API_URL_JOURNALS}/`, configJournal)
        .then(response => {
          ...
          setDataFourn(response.data.results[0].accounts);
...
// ----------------------get fournisseur---------------------------------------
    ...
    const fournisseur = dataFourn.filter((e) => e.code,);
    const tableCode = fournisseur.map(e => e);

    const list = tableCode.filter(item => ((item.code.toString().substr(0, 3) == account_Fournisseur[0]) || (item.code.toString().substr(0, 3) == account_Fournisseur[1])));
    ...
    const onChangeFournissur = (el: any) => {
      ...
      const dataFournisseur = dataFourn.filter((fil: any) => fil.id === state.compteFournisseur);
 ...
 //-----------------------get ttc and tva--------------------------------------
    ...
    const onChangeTotalTTC = (e: any) => {
      ...
      console.log('state.tva ',state.tva )
      console.log(totht)
      console.log(tottva)
      form.setFieldsValue({debit_amountt: tottva.toFixed(2)});
      form.setFieldsValue({debit_amounts: totht.toFixed(2)});
...
//------------------------------get contre partie-------------------------
    ...
    const listeContre = tableContre.filter(el => (el.code.toString().substr(0, 2) == account_compte[0]) ||
      ...
      (el.code.toString().substr(0, 2) == account_compte[3]));
...
//-----------------------------get status------------------------------------
    ...
    const onFinish = (values: {
      status: any; ficalyear: string; journal: any; category: any; date: any; due_date: string; label: string;
      ...
      axios.post(`${API_URL_TRANSACTIONS}/`, {
        ...
        .then(response => {
          ...
          setAlert(['add', values.account_fournisseur]);
           form.resetFields()
    ...
    return (
      ...
      <Card>
         <Title level={2}>Enregistrer une facture fournisseur</Title>
         <Divider/>
    ...
    {(alert[0] === 'add' && state.isVisible) ?
          <Alert message={`la facture a été créé avec succès`} type="success"  onClose={setTimeout(()=>{
      ...
      <br />
        ...
        >
          ...
          <Form.Item
            ...
            >
            <Input disabled={inactif} type="number" style={{width: 260}} onChange={onChangeTotalTTC}
          ...
          <Form.Item
            ...
            >
            <Input disabled={true} style={{width: 260}} type="text" value={state.totalht}/>
          </Form.Item>
          ...
          <Form.Item
            ...
            label="Total TVA"
          >
            <Input disabled={true} style={{width: 260}} type="text"/>
          </Form.Item>
          ...
          <Form.Item name="status" label="Status">
            ...
            <Button htmlType="submit" type="primary" style={{display: 'flex'}} disabled>
              ...
              {/*<Button htmlType="submit" type="primary" style={{ display: 'flex' }} onClick={onResert}>*/}
              {/*  Vider<SaveOutlined />*/}
              {/*</Button>*/}
            </div>
          </Form.Item>
