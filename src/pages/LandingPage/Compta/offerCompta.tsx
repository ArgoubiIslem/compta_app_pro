...
function AppOffersCompta() {
    ...
    useEffect(() => {
        ...
        .then(response => {
          ...
          .then(response2 => {
            let features=[]
            response.data.results.filter(result => result.module.code.includes("Compta")).map(offer => {
    ...
    <div id="pricing" className="block pricingBlock bgGray">
      <div className="container-fluid">
        <div className="titleHolder">
          <h2>Offres de Souscription Comptabilité</h2>
               ...
               {offerList.map(offer => {
                 return (
                   <td key={offer.id} className="td-buttons checked-option">
                     <div>{offer.name}</div>
                     <div className="left-part">{offer.price}	</div>
...
export default AppOffersCompta;
